...
/*******************DMA3传输*************************/
void DMA_transmit_init1(void *SADDR, void *DADDR,uint32 count,uint8 source)
{
        /* 开启时钟 */
       SIM_SCGC7 |= SIM_SCGC7_DMA_MASK;                        //打开DMA模块时钟
       ...
       DMAMUX_CHCFG3=0;                           //初始化前关闭该通道
       
       DMAMUX_CHCFG3=DMAMUX_CHCFG_SOURCE(source); //DMA通道1对应32号DMA请求，即FTM1 Channel 0 
       
       DMA_TCD3_CITER_ELINKNO=DMA_CITER_ELINKNO_CITER(count);//当前主循环次数,采集 点数 
       DMA_TCD3_BITER_ELINKNO=DMA_BITER_ELINKNO_BITER(count);//起始主循环次数，采集点数 
       
       DMA_TCD3_SADDR  =(u32)SADDR;
      
       DMA_TCD3_DADDR = (u32)DADDR;     
       // 设置目的地址
       
       
       DMA_TCD3_NBYTES_MLNO=DMA_NBYTES_MLNO_NBYTES(1);      //每次读取一字节
       
       DMA_TCD3_SOFF=0;        //每次传送源地址不变
       DMA_TCD3_SLAST=0;       //主循环结束后源地址不变 
       
       DMA_TCD3_DLASTSGA=0;  //主循环结束后即一列的数据采集完毕保持目的地址，自动指向下一行数据
       DMA_TCD3_DOFF=1;      //每次写目的地址加1,需要根据定义的数组类型来定
       
       DMA_TCD3_ATTR=DMA_ATTR_SSIZE(0)|DMA_ATTR_DSIZE(0);//源数据8bit，目的数据8bit 
       
       
       DMA_TCD3_CSR= DMA_CSR_DREQ_MASK;         //DMA通道3主循环结束后停止硬件请求
       
       DMA_TCD3_CSR|=DMA_CSR_INTMAJOR_MASK;    //使能DMA3中断 
       
       DMAMUX_CHCFG3|=DMAMUX_CHCFG_ENBL_MASK; //DMA通道3使能 
       
       DMA_ERQ |= DMA_ERQ_ERQ3_MASK;          //使能通道3的服务请求
       
       //set_irq_priority (63, 0);              //设置中断优先级
       DMA_EN(DMA_CH3);
       
       enable_irq(3);                        //开DMA通道3主循环结束中断  
